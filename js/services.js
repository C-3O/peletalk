...
.factory('sql', ['$rootScope','$cordovaSQLite', function($rootScope,$cordovaSQLite){
	...
	sqlObj.loadChats = function(my) {
		...
		var query = "SELECT logtable.logid FROM ( SELECT from_id as logid FROM chats UNION SELECT to_id as logid FROM chats )logtable where logid not like ? ";
		
        $cordovaSQLite.execute($rootScope.db, query,[my]).then(function(res) {
            if(res.rows.length > 0) {
				for (var i=0 ; i<res.rows.length; i=i+1) {
					sqlObj.chatlogs.push({
						id: res.rows.item(i).logid,
						name:  res.rows.item(i).logid,
						lastText: 'Last Text',
						face: 'img/ben.png'
					});
                }				
